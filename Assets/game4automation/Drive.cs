        ...
        private void SetPosition()
        {
            if (Direction == DIRECTION.Virtual)
            {
                ...
                if (useagx)
                {
                    ...
                    if (ReverseDirection)
                        dir = -1;
                    if (IsRotation)
                        agxlockcontroller.Position = dir*Mathf.Deg2Rad * (CurrentPosition+Offset);
                    else
                        agxlockcontroller.Position = dir*CurrentPosition+Offset;
            ...
            if (!UseInteract)
            {
                if (!_istransportsurface)
                {
                    ...
                    {
                        Quaternion localrot =
                            _localstartquat * Quaternion.Euler(_localdirection * (CurrentPosition + Offset));
                        if (MoveThisRigidBody)
                        {
                            if (!ReferenceEquals(transform.parent, null))
                            {
